...
struct ScrollOrderTitleView: View {
    ...
    var body: some View {
        LazyVStack(alignment: .leading){
            ...
            HStack {
                ...
                CustomText(title: dateToString(chart.orderItem.date), textColor: .black, textWeight: .semibold, textSize: 18)
                    .padding(.leading, 82)
            ...
            .onChange(of: chart.orderItem.date, perform: { value in
                ...
                if hour < 11 || (hour == 11 && minute < 30) {
                    chart.orderItem.date = calendar.date(bySettingHour: 11, minute: 30, second: 0, of: value) ?? Date()
                } else if hour > 19 || (hour == 19 && minute > 30) {
                    chart.orderItem.date = calendar.date(bySettingHour: 19, minute: 30, second: 0, of: value) ?? Date()
                } else if minute % 30 != 0 {
                    let roundedMinute = (minute / 30) * 30 + (minute % 30 > 15 ? 30 : 0)
                    chart.orderItem.date = calendar.date(bySettingHour: hour, minute: roundedMinute, second: 0, of: value) ?? Date()
                }
                
                if excludedDays.contains(day - 1) {
                    chart.orderItem.date = calendar.date(byAdding: .day, value: 1, to: value) ?? Date()
                }
            })
    ...
    func icepackCheck() {
        if icepack["보냉팩"] == false && icepack["보냉백"] == false {
            chart.orderItem.icePack = IcePack.none
        }
    }
}
