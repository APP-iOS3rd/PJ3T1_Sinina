...
class OrderDateViewModel: ObservableObject {
    @Published var dateValues: [DateValue] = []
    @Published var allOrderData: [OrderItem] = []
    
    lazy var targetDate = dateValues[0].date.withoutTime()
    var db: Firestore!
    ...
    func fetchData() {
        ...
        let query = db.collection("CurrentOrders").whereField("date", isEqualTo: targetDate)
